...
static void mavlink_test_drone_model(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_drone_model_t packet_in = {
		"ABCDEFGHIJKLMN"
    };
	mavlink_drone_model_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        
        	mav_array_memcpy(packet1.model, packet_in.model, sizeof(char)*15);
        

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_drone_model_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_drone_model_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_drone_model_pack(system_id, component_id, &msg , packet1.model );
	mavlink_msg_drone_model_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_drone_model_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.model );
	mavlink_msg_drone_model_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_drone_model_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_drone_model_send(MAVLINK_COMM_1 , packet1.model );
	mavlink_msg_drone_model_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_ardupilotmega(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_drone_model(system_id, component_id, last_msg);
}
